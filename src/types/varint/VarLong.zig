...
pub const VarLong = struct {
    ...
    pub fn read(self: *BinaryStream, _: ?Endianess) u64 {
        var value: u64 = 0;
        var size: u4 = 0;
    ...
    /// The endianess parameter is ignored for VarLong as it uses a special encoding.
    pub fn write(self: *BinaryStream, mut_value: u64, _: ?Endianess) void {
        var value = mut_value;
...
test "VarLong read/write" {
    ...
    VarLong.write(&stream, test_value, null);

    // Reset offset to read from the beginning
    stream.offset = 0;

    // Test reading the value
    const read_value = VarLong.read(&stream, null);
